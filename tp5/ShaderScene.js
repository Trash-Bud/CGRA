...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/custom.vert", "shaders/custom.frag"),
			new CGFshader(this.gl, "shaders/customGreyScale.vert", "shaders/customGreyScale.frag"),
	...
	update(t) {
		...
		if (this.selectedExampleShader == 6)
			...
			this.testShaders[9].setUniformsValues({ timeFactor: Math.sin(t / 100 % 100)*this.scaleFactor });
        }
		else if (this.selectedExampleShader == 11){
			this.testShaders[11].setUniformsValues({ timeFactor: t / 100 % 100/100 });
		}
	}
